...
public class ScheduledInterpolatingController implements Sendable, AutoCloseable {
   ...
   * @throws IllegalArgumentException if the lengths of the kp, ki, kd, and fuzzyStates arrays are
   *     not equal
   ...
   */
  public ScheduledInterpolatingController(
      double[] kp, double[] ki, double[] kd, double[] fuzzyStates) {
  ...
  public ScheduledInterpolatingController(
      double[] kp, double[] ki, double[] kd, double[] fuzzyStates, double period) {

    if (fuzzyStates.length != kp.length
        || fuzzyStates.length != ki.length
        || fuzzyStates.length != kd.length) {
   ...
   * @param ki integral gain parameters
   * @param kd derivative gain parameters
   * @throws IllegalArgumentException if the values of kp, ki, or kd are negative
   */
  ...
  /**
   * Sets the P gain for a specific fuzzy state
   *
   * @param fuzzyIndex the index of the fuzzy state
   * @param kp the proportional gain parameter
   * @throws IllegalArgumentException if fuzzyIndex is out of bounds
   * @throws IllegalArgumentException if kp &lt; 0
   */
  public void setIndexedP(int fuzzyIndex, double kp) {

    if (fuzzyIndex < 0 || fuzzyIndex >= m_fuzzyStates.length) {
      throw new IllegalArgumentException("Fuzzy index out of bounds");
    }
    if (kp < 0) {
      throw new IllegalArgumentException("P gain must be non-negative");
    }
    m_kpMap.put(m_fuzzyStates[fuzzyIndex], kp);
  }
  ...
  /**
   * Returns true if the error is within tollerance of the setpoint. The error tolerance defaults to
   * 0.05 while the error derivative tolerance defaults to {@link Double.POSITIVE_INFINITY}.
   *
  ...
  public void setIntegratorRange(double minimum, double maximum) {
    if (minimum >= maximum) {
      throw new IllegalArgumentException(
          "Minimum integral value must be less than maximum integral value");
    }
  ...
  /**
   * Returns the next output of the ScheduledController based on the current measurement and
   * setpoint.
   *
